...
================================================= */
const PLANT_GAS_URL =
  'https://script.google.com/macros/s/AKfycbzfUbGWXNdxPdfW7R1c6H03X2g-711TN9L7I4Vn4vS1eyZlIIJtfsulAOz0Yl30-X1LpQ/exec';
...
function switchPage(source) {
  ...
  document.getElementById('source-selector').textContent =
    currentPageName + ' ▼';
...
async function fetchPlantData() {
  try {
    ...
    document.getElementById('plant-pm25-value').textContent =
      (data.pm25 ?? '--') + ' μg/m³';
    document.getElementById('plant-humidity').textContent =
      (data.humidity ?? '--') + ' %';
    document.getElementById('plant-temperature').textContent =
      (data.temperature ?? '--') + ' °C';
    document.getElementById('plant-soil-humidity').textContent =
      (data.soil_moisture ?? '--') + ' %';
    document.getElementById('plant-co2').textContent =
      (data.co2 ?? '--') + ' ppm';
...
async function fetchData() {
  try {
    ...
    const url = `https://pm25.lass-net.org/data/last.php?device_id=${cfg.deviceId}`;
    const res = await fetch(url);
    const data = await res.json();

    document.getElementById('pm25-value').textContent =
      (data.s_d0 ?? '--') + ' μg/m³';
    document.getElementById('temperature-card').textContent =
      (data.s_t0 ?? '--') + ' °C';
    document.getElementById('humidity-card').textContent =
      (data.s_h0 ?? '--') + ' %';
    document.getElementById('windspeed-card').textContent =
      (data.s_w0 ?? '--') + ' m/s';
    document.getElementById('sunlight-card').textContent =
      (data.s_lux0 ?? '--') + ' lux';
    document.getElementById('co2-card').textContent =
      (data.s_co2 ?? '--') + ' ppm';
    document.getElementById('tvoc-card').textContent =
      (data.s_tvoc ?? '--') + ' ppb';
...
function updateClock() {
  ...
  document.getElementById('date-display').textContent =
    `${days[now.getDay()]} ${now.getMonth()+1}/${now.getDate()}/${now.getFullYear()}`;
